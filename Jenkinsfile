...
pipeline{
	...
	stages
	{
        ...
        stage('Build') 
		{
			steps {
				echo "------------>Build<------------"
				sh 'chmod +x ./galeria/gradlew'
				sh './galeria/gradlew --b ./galeria/build.gradle clean'
				sh './galeria/gradlew --b ./galeria/build.gradle build -x test'
			}
		}  
	}
